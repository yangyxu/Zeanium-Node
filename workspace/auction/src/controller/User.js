    ...
    return zn.Controller('user',{
        ...
        methods: {
            ...
            login: {
                ...
                value: function (request, response, chain){
                    this._store.command.query("select * from zn_auction_user where (phone='{0}' or name='{0}' or email='{0}') and password='{1}'".format(request.getValue('token'),request.getValue('password'))).then(function (data){
                        var user = data[0];
                        if(user){
                            if(+user.status==32){
